...
//     try {
//       const response = await axios.post("http://localhost:8080/api/foods/login", {
//         email,
//         password,
//       });
...
//             <Link to="/signup" className="fw-bold" style={{ color: "#ff6600" }}>
//               Sign Up
//             </Link>
//           </p>
//         </div>
...
function Login() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("http://localhost:8080/api/foods/login", {
        method: "POST",
      ...
      loginUser({ name: data.name }); // save in context
      navigate("/");
    } catch (err) {
      console.error(err);
    ...
    <div className="vh-100 d-flex flex-column flex-md-row">
      ...
      <div className="d-flex align-items-center justify-content-center flex-grow-1" style={{ background: "linear-gradient(135deg, #ffffff, #f8f9fa)", padding: "40px" }}>
        <div className="card shadow-lg p-4 rounded-4" style={{ maxWidth: "420px", width: "100%", border: "none" }}>
          ...
          <h2 className="text-center mb-4 fw-bold" style={{ color: "#ff6600" }}>Login</h2>
          {error && <p className="text-danger text-center">{error}</p>}
          <form onSubmit={handleLogin}>
            ...
            <button type="submit" className="btn w-100 py-2 fw-bold rounded-3" style={{ backgroundColor: "#ff6600", border: "none", color: "white", fontSize: "16px" }}>Login</button>
          </form>
          <p className="text-center mt-3">
            Don't have an account? <Link to="/signup" className="fw-bold" style={{ color: "#ff6600" }}>Sign Up</Link>
          </p>
        </div>
      </div>
    </div>
